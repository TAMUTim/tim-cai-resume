...
\section{Projects} 
  \resumeProjectListStart 
    \resumeProjectItem{Found in Translation}{Python, JavaScript, Slack Bolt, Flask, Node.js, Co:Here, Pinecone, Azure}
      \resumeItemListStart
        ...
        \resumeItemDescription{Utilized \textbf{Flask} to train \textbf{CoHere} models on the \textbf{Google GoEmotion dataset}, with storage through \textbf{Pinecone}, and everything hosted on \textbf{Azure}}
        \resumeItemDescription{Developed interface with \textbf{Slack Bolt API}, and improving time efficiency by up to \textbf{50\%} by designing a \textbf{multithreaded} system of back-end calls,}
      \resumeItemListEnd
  \resumeProjectItem{Mock Shell}{C++, PHP}
      \resumeItemListStart
        ...
        \resumeItemDescription{Developed and designed systems for piping, file I/O redirection, background processes, with the option of multithreading for each}
      \resumeItemListEnd
  \resumeProjectItem{makea.horse}{Javascript, HTML, CSS}
      \resumeItemListStart
        \resumeItemDescription{Fun website displaying generated image at a certain time created for a Speed Development Hackathon}
        \resumeItemDescription{Written in a single Javascript file as emphasis on \textbf{monolithic} architecture and deployed through \textbf{Netlify}} 
      \resumeItemListEnd
   \resumeProjectItem{The Galactic Algorithm}{Python, tensorflow, keras, PIL}
      \resumeItemListStart
        ...
        \resumeItemDescription{Designed three separate heuristics which combined into the end algorithm, with a final accuracy of \textbf{97\%}}
        \resumeItemDescription{Utilized augmented \textbf{Nearest Bodies} algorithm for object detection, and further reduced inaccuracy by matching edges} 
      \resumeItemListEnd
   \resumeProjectItem{Personal Server}{C++, Twilio, Pinecone}
      \resumeItemListStart
        \resumeItemDescription{Simple multithreaded server that reads/writes various inputs regarding personal calendar, habits, and data through \textbf{Twilio SMS}}
        \resumeItemDescription{Designed with \textbf{multithreading} in mind, has capacity of handling single requests up to \textbf{1.2 GB}, with data stored on \textbf{Pinecone}}
      \resumeItemListEnd
    \resumeProjectItem{Small Distributed Social Network}{C++, gRPC, glog, cmake}
      \resumeItemListStart
        \resumeItemDescription{Small distributed implementation of a social network service utilizing Chubby lock system and server clusters, and built with \textbf{grpc}. Designed to be scalable, fault tolerant, and highly available where failures are handled transparently to users. }
        \resumeItemDescription{Server clusters split into master / slave, with \textbf{Chubby} running as coordinator, close to \textbf{99\%} uptime.}
      \resumeItemListEnd
